	...
	rows, err := s.DB.Query(`
		SELECT c.id, c.title, c.slug, c.description, c.cover_photo, c.sort_order,
			...
			(SELECT COUNT(*) FROM collection_photos cp
			 ...
			 WHERE cp.collection_id = c.id AND p.published = true) AS photo_count
		FROM collections c
...
func (s *Server) handleGetCollection(w http.ResponseWriter, r *http.Request) {
	...
	// Load published photos in this collection
	rows, err := s.DB.Query(`
		SELECT p.id, p.title, p.description, p.slug, p.film_stock, p.camera, p.lens, p.taken_at,
			p.published, p.variants, p.width, p.height, p.file_size, p.blur_hash,
			p.sort_order, p.created_at, p.updated_at
		FROM photos p
		JOIN collection_photos cp ON cp.photo_id = p.id
		WHERE cp.collection_id = $1 AND p.published = true
